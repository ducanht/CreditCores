...
import { Zap, CheckCircle2, ChevronRight, AlertTriangle, ArrowLeft } from 'lucide-react';
...
export default function DebitBatchCreateModal({
  ...
  const handleNextToStep2 = (e) => {
    ...
    const list = activeRegs.map((reg) => {
      ...
      const tongDuNo = custContracts.reduce((sum, c) => sum + (c.duNo || 0), 0);
      const laiPhatSinh = Math.round(
        custContracts.reduce((sum, c) => sum + ((c.duNo || 0) * (c.laiSuat || 9.5) / 100 / 12), 0)
      );
        ...
        <div className="modal-content card-modern p-4">
          <div className="modal-header border-0 pb-0">
            <h5 className="modal-title fw-bold text-dark font-heading d-flex align-items-center gap-2">
              ...
              {step === 1 ? 'Bước 1: Chọn Kỳ & Tháng Trích Nợ' : 'Bước 2: Chọn Lọc Khách Hàng & Chỉnh Sửa Số Tiền Trích'}
            </h5>
          ...
          {step === 1 ? (
            <form onSubmit={handleNextToStep2}>
              <div className="modal-body py-3">
                ...
                <div className="p-3 bg-light rounded-3 border small text-muted">
                  <strong>💡 Cơ Chế Tự Động Tính Toán:</strong> Hệ thống sẽ tự động quét danh sách ủy quyền kỳ này, cộng dồn Lãi phát sinh + Gốc đến hạn + Nợ tồn kỳ trước và chuyển sang bảng duyệt.
                </div>
              </div>
            ...
            <div className="modal-body py-3">
              ...
              <div className="table-responsive border rounded-3 bg-white mb-3" style={{ maxHeight: 380, overflowY: 'auto' }}>
                  ...
                  <tbody>
                    {eligibleList.length > 0 ? (
                      eligibleList.map((item) => {
                        ...
                        return (
                          <tr key={item.maKH} className={isChecked ? '' : 'table-light text-muted'}>
                            ...
                            <td className="text-end text-danger num-tabular">{formatCurrencyVN(item.laiPhatSinh)}</td>
                      ...
                      <tr>
                        <td colSpan="8" className="text-center text-muted py-4">
                          Không tìm thấy khách hàng nào đăng ký trích nợ cho Kỳ {formData.kyTrich}.
                        </td>
                      </tr>
                    )}
                  </tbody>
