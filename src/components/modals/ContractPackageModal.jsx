...
export default function ContractPackageModal({
  ...
  const d = {
    ...
    MaKH: customer?.maKH || '',
    SoCCCD: customer?.cccd || customer?.gttt || '',
    NgayCapCCCD: customer?.ngayCap || '',
    NoiCapCCCD: customer?.noiCap || '',
    NgaySinh: customer?.ngaySinh || '',
    DiaChi: customer?.diaChi || '',
    DienThoai: customer?.dienThoaiDD || customer?.dienThoai || '',
    SoTV: customer?.soTV || '',
    SoTK_CASA: customer?.soTK || '',

    // Khoản vay
    SoHDTD: contract?.soHDTD || '',
    TienVay: Number(contract?.tienVay || contract?.duNo || 0),
    LaiSuat: Number(contract?.laiSuat || 0),
    NgayVay: contract?.ngayVay || getTodayVN(),
    DenHan: contract?.denHan || '',
    SoThangVay: contract?.soThangVay || 0,
    MucDichVay: contract?.moTaVay || '',
    PhuongThucTraGoc: 'Trả gốc định kỳ hàng tháng, lãi tính trên dư nợ thực tế',

    // Tài sản thế chấp (TSBD_CORE)
    SoGCN: collateral?.soGCN || '',
    SoVaoSoCapGCN: collateral?.soVaoSoCapGCN || '',
    NgayCapGCN: collateral?.ngayCapGCN || '',
    NoiCapGCN: collateral?.noiCapGCN || '',
    ChuSoHuu: collateral?.chuSoHuu || customer?.hoTen || '',
    CCCD_ChuTS: collateral?.cccdChuTS || customer?.cccd || '',
    ThuaDatSo: collateral?.thuaDatSo || '',
    ToBanDoSo: collateral?.toBanDoSo || '',
    DiaChiThuaDat: collateral?.diaChiThuaDat || '',
    DienTich: collateral?.dienTich || 0,
    GiaTriDinhGiaQTD: Number(collateral?.giaTriDinhGiaQTD || 0),
    SoTienDamBaoToiDa: Number(collateral?.soTienDamBaoToiDa || 0),
