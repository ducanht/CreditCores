...
FROM TD_KHE_UOC A
INNER JOIN TD_HOP_DONG_TD D
    ON A.MA_HDTD = D.MA_HDTD
INNER JOIN DC_KHACH_HANG B
    ON B.MA_KHACH_HANG = D.MA_KHACH_HANG
INNER JOIN KT_TAI_KHOAN C
    ON C.SO_TAI_KHOAN = A.SO_TAI_KHOAN
INNER JOIN DC_KHU_VUC KV
    ON B.MA_KHU_VUC = KV.MA_KHU_VUC
INNER JOIN vwTD_SAN_PHAM SP
    ON SP.MA_SAN_PHAM = A.MA_SAN_PHAM
INNER JOIN DC_LOAI_VAY LV
